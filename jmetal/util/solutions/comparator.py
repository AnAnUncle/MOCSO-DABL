...
class AggregateComparator(Comparator):
    ...
    def __dominance_test(self, solution1: Solution, solution2: Solution) -> float:
        ...
        value1 = numpy.mean(solution1.objectives)
        value2 = numpy.mean(solution2.objectives)
        if value1 != value2:
            if value1 < value2:
                best_is_one = 1
            if value1 > value2:
...
# 原版 constraint_comparator=SolutionAttributeComparator('overall_constraint_violation', False)):
class EpsilonDominanceComparator(DominanceComparator):
    def __init__(self,
                 epsilon: float,
                 constraint_comparator=OverallConstraintViolationComparator()):
        super(EpsilonDominanceComparator, self).__init__(constraint_comparator)
        self.epsilon = epsilon
        self.name = "EpsilonDominanceComparator"
    ...
    def __dominance_test(self, solution1: Solution, solution2: Solution):
        ...
        for i in range(solution1.number_of_objectives):
            try:
                ...
                print("error")
                print(solution1.objectives[i], self.epsilon)
                print(solution1.objectives[i], self.epsilon)
                sys.exit(0)
...
class EpsilonPlusDominanceComparator(DominanceComparator):

    def __init__(self,
                 epsilon: float,
                 constraint_comparator=SolutionAttributeComparator('overall_constraint_violation', False)):
        super(EpsilonPlusDominanceComparator, self).__init__(constraint_comparator)
        self.epsilon = epsilon
