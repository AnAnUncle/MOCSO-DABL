...
class NonDominatedSolutionListArchive(Archive[S]):
        ...
        if len(self.solution_list) == 0:
            ...
            for index, current_solution in enumerate(list(self.solution_list)):
                ...
                #         break
                    ...
                    is_near = False
                    for i in range(current_solution.number_of_objectives):
                        if numpy.abs(solution.objectives[i] - current_solution.objectives[i]) <= 0.0075:
                            is_near = True
                            break
                    if is_near:
                        if numpy.mean(solution.objectives) <= numpy.mean(current_solution.objectives):
                            del self.solution_list[index - number_of_deleted_solutions]
                            number_of_deleted_solutions += 1
                        else:
                            is_contained = True
    ...
    def delete(self, i) -> bool:
        self.solution_list[i].attributes['deleted'] = True

    def update(self):
        temp = []
        for solution in self.solution_list:
            try:
                solution.attributes['deleted']
            except BaseException:
                temp.append(solution)
        self.solution_list = temp
...
class CrowdingDistanceArchive19(BoundedArchive[S]):

    def __init__(self,
                 maximum_size: int):
        super(CrowdingDistanceArchive19, self).__init__(
