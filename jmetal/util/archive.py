...
class NonDominatedSolutionListArchive(Archive[S]):
    ...
    # epsilon = 0.00075
    def __init__(self, dominance_comparator=EpsilonDominanceComparator(epsilon=0.0075)):
            ...
            for index, current_solution in enumerate(list(self.solution_list)):
                ...
                if is_dominated_flag == -1:
                    ...
                    if solution.objectives == current_solution.objectives:
                        is_contained = True
                        break
                    # # 个体非支配，则依据支配比较算子的不同采用不同的判断是否包含标准
                    # if self.comparator.name == "EpsilonDominanceComparator":7
                    #     if euclidean(solution.objectives, current_solution.objectives) <= 0.0075:
                    #         is_contained = True
                    #         break
                    # else:
                    #     if solution.objectives == current_solution.objectives:
