...
class MOCSO(ChickenSwarmOptimization):
    ...
    # def select_global_best(self, particle) -> FloatSolution:
    #     flag = 10
    #     mean = []
    #     while flag and self.evaluations > 14000:
    #         global_best = self.select_global_best_random()
    #         # 判断备选的全局最优是否被采纳的阈值
    #         euclidean_distance = euclidean(particle.objectives, global_best.objectives)
    #         # euclidean_distance > numpy.sqrt(2) / 10
    #         # mean.append(euclidean_distance)
    #
    #         if euclidean_distance < numpy.sqrt(particle.number_of_objectives) / 10:
    #             return global_best
    #         flag = flag - 1
    #     global_best = self.select_global_best_random()
    #     return global_best
