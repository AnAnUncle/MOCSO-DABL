...
def generate_summary_from_experiment_psp(input_dir: str, quality_indicators: List[QualityIndicator],
                ...
                for indicator in quality_indicators:
                    ...
                    if hasattr(indicator, 'reference_front'):
                        if Path(reference_front_file).is_file():
                            indicator.reference_front = read_port_reference(reference_front_file)
                        else:
...
def generate_summary_from_experiment_3d(input_dir: str, quality_indicators: List[QualityIndicator],
                                        reference_fronts: str = ''):
    """ Compute a list of quality indicators. The input data directory *must* met the following structure (this is generated
    automatically by the Experiment class):
    ...
    * <base_dir>
    ...
    with open('QualityIndicatorSummary_3d.csv', 'w+') as of:
        of.write('Algorithm,Problem,ExecutionId,IndicatorName,IndicatorValue\n')
    ...
    for dirname, _, filenames in os.walk(input_dir):
        for filename in filenames:
            ...
            #         content = content_file.read()
            #
            #     with open('QualityIndicatorSummary_3d.csv', 'a+') as of:
            #         of.write(','.join([algorithm, problem, run_tag, 'Time', str(content)]))
            #         of.write('\n')
...
def generate_latex_tables(filename: str, output_dir: str = 'latex/statistical'):
    ...
    max, min = pd.DataFrame(), pd.DataFrame()
    ...
    for algorithm_name, subset in df.groupby('Algorithm', sort=False):
        ...
        # Compute min and max
        min_ = subset.groupby(level=[0, 1]).min()
        min = pd.concat([min, min_], axis=1)

        max_ = subset.groupby(level=[0, 1]).max()
        max = pd.concat([max, max_], axis=1)
    ...
    for indicator_name, subset in std.groupby('IndicatorName', sort=False):
        ...
        subset = min.groupby('IndicatorName', sort=False).get_group(indicator_name)
        subset.index = subset.index.droplevel(1)
        subset.to_csv(os.path.join(output_dir, 'Best-{}.csv'.format(indicator_name)), sep='\t', encoding='utf-8')

        subset = max.groupby('IndicatorName', sort=False).get_group(indicator_name)
        subset.index = subset.index.droplevel(1)
        subset.to_csv(os.path.join(output_dir, 'Worst-{}.csv'.format(indicator_name)), sep='\t', encoding='utf-8')
