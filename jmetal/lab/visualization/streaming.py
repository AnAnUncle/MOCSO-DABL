...
class StreamingPlot:
    ...
    def plot(self, front):
        ...
        if self.reference_point:
            for point in self.reference_point:
                self.scp, = self.ax.plot(*[[p] for p in point], c='r', ls='None', marker='*', markersize=18)
        ...
        self.sc, = self.ax.plot(*[points[column].tolist() for column in points.columns.values],
                                ls='None', marker='o', markersize=4, color="b")
    ...
    def update(self, front: List[S], reference_point: list = None) -> None:
        ...
        # If any new reference point, plot.新增三维参考点集设置。可以将想展示的点集设为参考点动态显示
        if reference_point and dimension == 2:
            self.scp.set_data([p[0] for p in reference_point], [p[1] for p in reference_point])
        elif reference_point and dimension == 3:
            a = pd.DataFrame(reference_point)
            self.scp.set_data(a[0], a[1])
            self.scp.set_3d_properties(a[2])
        # Re-align the axis
        self.ax.relim()
